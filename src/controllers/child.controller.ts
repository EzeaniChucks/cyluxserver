...
export class ChildController {
  ...
  triggerCommand = async (req: any, res: Response) => {
    try {
      ...
      if (type === 'HIDE_ICON') { child.iconHidden = true; await this.childRepo.save(child); }
      if (type === 'SHOW_ICON') { child.iconHidden = false; await this.childRepo.save(child); }
      ...
      if (type === 'DISABLE_SETTINGS_GUARD') {
        ...
        child.blockedApps = child.blockedApps.filter((p: string) => p !== 'com.cyluxchild');
        await this.childRepo.save(child);
      }

      return ApiResponse.success(res, { ...command, iconHidden: child.iconHidden, settingsGuardEnabled: child.settingsGuardEnabled, blockedApps: child.blockedApps }, 'Command queued');
    } catch (error: any) {
      return ApiResponse.error(res, error.message);
    }
  };
