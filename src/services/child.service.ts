...
export function deviceLocalDateString(date: Date, timezone?: string | null): string {
  if (timezone) {
    try {
    ...
    } catch { /* invalid IANA name — fall through */ }
  }
  return date.toISOString().slice(0, 10);
}

/** Returns minutes elapsed since device-local midnight (minimum 1). Falls back to UTC. */
export function minutesSinceLocalMidnight(date: Date, timezone?: string | null): number {
  if (timezone) {
    try {
...
export class ChildService {
  ...
  async processHeartbeat(childId: string, data: any) {
    const result = await AppDataSource.transaction(async (manager) => {
      ...
      let incomingAppUsage = data.appUsage as Record<string, number> | undefined;
      if (dayRolledOver && incomingAppUsage && typeof incomingAppUsage === 'object') {
        const capMinutes = minutesSinceLocalMidnight(now, deviceTz);
        incomingAppUsage = Object.fromEntries(
          Object.entries(incomingAppUsage).map(([pkg, mins]) => [
            pkg,
      ...
      if (data.apps && Array.isArray(data.apps)) {
        ...
        const currentApps: any[] = (updates.appUsage as any[] | undefined) || child.appUsage || [];
        ...
        updates.lastInventoryScan = now;
        const currentApps: any[] = updates.appUsage
          ? [...(updates.appUsage as any[])]
  ...
  async processLogBatch(logs: any[], authenticatedDeviceId?: string) {
    return await AppDataSource.transaction(async (manager) => {
      ...
      if (notifLogs.length > 0) {
        ...
        const logRepo = manager.getRepository(AuditLogEntity);
        const existing = await logRepo.find({
          where: notifLogs.map((l: any) => ({
            childId: l.childId,
            actionType: 'NOTIFICATION_RECEIVED',
            details: l.details,
            timestamp: MoreThan(sixtySecondsAgo),
          })),
