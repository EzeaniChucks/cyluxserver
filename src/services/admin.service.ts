...
export class AdminService {
  ...
  async seedFirstAdmin(data: { email: string; password: string; name: string }) {
    const existing = await adminRepo().count();
    if (existing > 0) throw new Error('Admin already seeded');
    const passwordHash = await bcrypt.hash(data.password, 12);
    const admin = adminRepo().create({ ...data, passwordHash, role: 'superadmin' });
    return adminRepo().save(admin);
  }

  async login(email: string, password: string) {
    if (!ADMIN_JWT_SECRET) throw new Error('ADMIN_JWT_SECRET not configured');
    const admin = await adminRepo().findOne({ where: { email: email.toLowerCase().trim() } });
    if (!admin || !admin.isActive) throw new Error('Invalid credentials');
    const valid = await bcrypt.compare(password, admin.passwordHash);
    if (!valid) throw new Error('Invalid credentials');
    admin.lastLoginAt = new Date();
    await adminRepo().save(admin);
    const token = jwt.sign({ id: admin.id, role: admin.role }, ADMIN_JWT_SECRET, { expiresIn: '8h' });
  ...
  async getPlatformStats() {
    ...
    const trialingSubs = await subRepo().count({ where: { status: 'trialing' } });
    const totalReferrals = await referralRepo().count();
    const totalConversions = await referralRepo().count({ where: { status: 'subscribed' } });
    ...
    for (const row of activeSubsByPlan) {
      ...
      const planConfig = PLAN_LIMITS[row.plan as PlanId];
  ...
  async getParents(opts: { page?: number; limit?: number; search?: string; plan?: string; status?: string }) {
  ...
  async getSubscriptions(opts: { page?: number; limit?: number; plan?: string; status?: string }) {
    const page = opts.page || 1;
    const limit = Math.min(opts.limit || 20, 100);
    const skip = (page - 1) * limit;
  ...
  async updatePlanConfig(planId: string, data: Partial<PlanConfigEntity>, adminId: string) {
    let config = await planConfigRepo().findOne({ where: { planId } });
    if (!config) throw new Error('Plan config not found');
  ...
  async getRevenue() {
    ...
    const byPlan = activeSubsByPlan.map(row => {
      const count = parseInt(row.count, 10);
      const price = PLAN_LIMITS[row.plan as PlanId]?.price || 0;
